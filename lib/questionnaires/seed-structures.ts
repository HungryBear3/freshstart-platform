/**
 * The questionnaire definitions this product seeds.
 *
 * Moved verbatim out of `lib/seed-questionnaires.ts` so they can be read WITHOUT
 * importing Prisma. `lib/forms/field-map-compatibility.ts` has to answer "does
 * this questionnaire field exist?" from the real definitions; a hand-copied list
 * would drift from what is actually seeded and the check would go quietly vacuous.
 *
 * Not the deployed database. `scripts/remediate-current-illinois-grounds.ts`
 * mutates a stored structure directly, so a deployed questionnaire can differ
 * from this file. Every statement derived from it is about the REPOSITORY.
 */
...
export const SEED_QUESTIONNAIRES: SeedQuestionnaire[] = [
  {
