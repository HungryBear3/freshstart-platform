...
import { CaseInsightsCard } from "@/components/dashboard/case-insights-card"
import { getCaseInsightsForUser, getCaseReadinessScore } from "@/lib/case/insights"
...
export default async function DashboardPage() {
  ...
  const hasCompletedPetition = progress.questionnaires.responses.some(
    (r) => r.formType === "petition" && r.status === "completed"
  )
  const hasCaseInfo = !!(caseInfo?.county || caseInfo?.filingDate)
  const hasGeneratedDocuments = progress.documents.count >= 1

  const [caseInsights, readinessScore, badges] = await Promise.all([
    getCaseInsightsForUser(user.id),
    getCaseReadinessScore(user.id),
  ...
  return (
    <DashboardLayout>
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        {/* Case Insights (personalized cost, timeline, readiness) */}
        {hasActiveSubscription && (
          <CaseInsightsCard insights={caseInsights} readiness={readinessScore} />
        )}
        ...
        {hasActiveSubscription && !hasCompletedOnboarding && (
          <GettingStartedChecklist
            hasCompletedPetition={hasCompletedPetition}
            hasCaseInfo={hasCaseInfo}
