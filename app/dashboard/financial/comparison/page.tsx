...
import { ArrowLeft, FileText, Upload, Loader2, AlertCircle, Edit, Trash2 } from "lucide-react";
...
import { SpouseAffidavitUpload } from "@/components/financial/spouse-comparison/spouse-affidavit-upload";
...
export default function FinancialComparisonPage() {
  ...
  const [pendingAction, setPendingAction] = useState<"enter" | "upload" | null>(null);
  ...
  return (
    <DashboardLayout>
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        {data?.hasUserData && !data.hasSpouseData && !showEntryForm && !showUpload && (
          <Card className="mb-8 border-amber-200 bg-amber-50">
            ...
            <CardContent>
              ...
              <div className="flex flex-wrap gap-3">
                <Button onClick={openAuthForEnter}>
                  <FileText className="h-4 w-4 mr-2" />
                  Enter Spouse&apos;s Financial Data
                </Button>
                <Button variant="outline" onClick={openAuthForUpload}>
                  <Upload className="h-4 w-4 mr-2" />
                  Upload Spouse&apos;s Affidavit
                </Button>
              </div>
            </CardContent>
          </Card>
      ...
      {showUpload && data?.hasUserData && (
        <div className="mb-8">
          <SpouseAffidavitUpload
            onUploadComplete={(docId) => handleUploadComplete(docId)}
            onCancel={() => {
              setShowUpload(false);
            }}
          />
          <p className="text-sm text-gray-600 mt-4">
            After uploading, you will enter the key figures from the affidavit for comparison.
          </p>
        </div>
      )}
