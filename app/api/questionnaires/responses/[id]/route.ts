...
export async function PUT(
  ...
  try {
    ...
    // Award first_petition badge when petition is completed
    let badgeEarned: string | null = null;
    if (sanitizedStatus === "completed" && existing.formType === "petition") {
      const { earned } = await awardBadge(session.user.id, "first_petition");
      if (earned) badgeEarned = "first_petition";
    }

    return NextResponse.json({ response, badgeEarned });
  } catch (error) {
