...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      if (!isOfficialForm) {
        switch (documentType) {
          ...
          default:
            ...
            const { earned: docBadge } = await awardBadge(session.user.id, "first_document");
            ...
            return NextResponse.json(
              {
                ...
                badgeEarned: docBadge ? "first_document" : null,
              },
              { status: 201 }
            );
        }
      ...
      const { earned: fallbackBadge } = await awardBadge(session.user.id, "first_document");
      ...
      return NextResponse.json(
        {
          ...
          badgeEarned: fallbackBadge ? "first_document" : null,
        },
        { status: 201 }
      );
    }
    ...
    // Award first_document badge
    const { earned: documentBadgeEarned } = await awardBadge(session.user.id, "first_document");
    ...
    return NextResponse.json(
      {
        ...
        badgeEarned: documentBadgeEarned ? "first_document" : null,
      },
      { status: 201 }
    );
  } catch (error) {
