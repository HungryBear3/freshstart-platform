...
11. [Phase 1 Critical Fixes (Ship-Ready)](#phase-1-critical-fixes-ship-ready-march-2026)
...
### Issue: Prisma Migrate with Supabase Pooler
**Error:** `prepared statement "s1" already exists` when running `prisma migrate dev`
**Problem:** Supabase transaction pooler (port 6543) doesn't support prepared statements required by Prisma migrations
**Solution:** Use **session mode** pooler (port 5432, same host) or direct connection. In `prisma.config.ts`, set `datasource.url` to `DIRECT_DATABASE_URL` when present:
```ts
datasource: {
...
# Session mode pooler (migrations) - port 5432, same host
DIRECT_DATABASE_URL="postgresql://...@aws-0-us-west-2.pooler.supabase.com:5432/postgres?sslmode=require"
```
**Lesson:** Session mode (5432) supports prepared statements; transaction mode (6543) does not.

### Issue: Prisma Migrate from Wrong Directory
**Error:** `Could not find Prisma Schema` when running `npx prisma migrate dev`
**Problem:** Command run from workspace root (`ai-dev-tasks`) instead of project directory (`newstart-il`)
**Solution:** Always run Prisma commands from the project directory:
```powershell
cd newstart-il
npx prisma migrate dev --name migration_name
```
Or specify schema: `npx prisma migrate dev --schema newstart-il/prisma/schema.prisma`

### Issue: SQL Editor When Schema Differs (Wrong Project or Table Names)
**Error:** `relation "users" does not exist` when running migration SQL in Supabase SQL Editor
**Problem:** Database may be a different project (e.g., overtaxed-platform vs newstart-il) or tables use different names (e.g., `"User"` vs `"users"` in PostgreSQL)
**Solution:** 
1. Run `SELECT table_schema, table_name FROM information_schema.tables WHERE table_schema = 'public'` to list actual table names
2. Create tables without foreign keys first if referenced tables don't exist
3. Use quoted identifiers for case-sensitive names: `REFERENCES "User"("id")` not `REFERENCES "users"("id")`
4. Add FKs only for tables that exist; skip FKs for missing tables (e.g., `case_info`)
...
### Critical: Deployment Source Repo

**www.freshstart-il.com deploys from `HungryBear3/freshstart-platform`, not from the FreshStart-IL monorepo.** All edits to `newstart-il` in the ai-dev-tasks repo must be synced to `freshstart-platform` before they reach production:

```powershell
cd "c:\Users\alkap\.cursor\FreshStart IL\ai-dev-tasks"
.\scripts\sync-newstart-to-freshstart-platform.ps1
```

- **Sync script:** `scripts/sync-newstart-to-freshstart-platform.ps1` — copies `newstart-il` contents into a temp dir, inits git, force-pushes to `freshstart-platform` `main`
- **Verify deployment:** https://www.freshstart-il.com/api/deploy-info — returns `{"app":"newstart-il","project":"FreshStart IL","favicon":"base64-in-metadata","updated":"..."}` when correct app is live

**Lesson:** If favicon or other fixes don't appear in production, check which repo Vercel is connected to. Pushing to FreshStart-IL alone will not update the live site.

### Favicon Placement for Next.js (and Why Base64 Won)

**Avoid `app/favicon.ico`:** Next.js file-based favicon emits `/favicon.ico?favicon.HASH.ico` and can override `metadata.icons`. CDN/browser caching can persist the wrong icon.

**Recommended (current implementation):** Embed favicon as base64 data URL in `metadata.icons`:
- `lib/favicon-base64.ts` — exports `FAVICON_DATA_URL`
- `app/layout.tsx` — `icons: { icon: { url: FAVICON_DATA_URL, type: "image/png", sizes: "32x32" } }`
- No external request, no cache issues
...
**Lesson:** When automatic Git deploys stop working, Deploy Hooks provide a reliable manual workaround. Also verify: Production Branch is set to `main`, Root Directory is `newstart-il` (or empty if freshstart-platform is a flat copy), and Git integration is connected to the correct repo (**freshstart-platform**, not FreshStart-IL).

---

## Phase 1 Critical Fixes (Ship-Ready) (March 2026)

Implemented security and data integrity fixes per the FreshStart IL Critical Fixes and Roadmap plan.

### 1. Unauthenticated File Access
**Issue:** Files API served any file under storage with no session check; paths could be guessed.
**Fix:** `getCurrentUser()` at start of GET handler; require auth; enforce path ownership (userId prefix). Only `prenups/{userId}/...` or `spouse-affidavits/{userId}/...` allowed.

### 2. Admin Routes Unprotected
**Issue:** `/admin/*` and `/api/admin/*` were open.
**Fix:** Added `role` to User model; migration `20260302000000_add_user_role`; middleware protects admin routes; `requireAdmin()` helper for API routes. Seed at least one admin: `UPDATE users SET role = 'admin' WHERE email = 'admin@example.com'`.

### 3. Seed Endpoints Open to All
**Issue:** All seed routes had zero auth.
**Fix:** `requireAdmin()` on seed-test-user, seed-legal-content, seed-questionnaires, seed-case, marketing-links. seed-test-user disabled in production.

### 4. Account Deletion Without Stripe Cancellation
**Issue:** User deleted without canceling Stripe subscription.
**Fix:** Before `prisma.user.delete()`, fetch Subscription and call `stripe.subscriptions.cancel(stripeSubscriptionId)` if present. Log Stripe errors but do not block deletion.

### 5. Contact Form XSS
**Issue:** User inputs (name, email, subject, message, category) injected into HTML without escaping.
**Fix:** `escapeHtml()` in `lib/security/validation.ts`; escape all inputs before use in `sendContactFormEmails`. For message: `escapeHtml(message).replace(/\n/g, "<br>")`.

### 6. Incomplete Data Export
**Issue:** Export omitted children, parentingPlan, spouseFinancialRecords, subscription, payments.
**Fix:** Extended Prisma `include` to add these relations; added nested `financialData` (income, expenses, assets, debts) include. Subscription export excludes Stripe IDs; payments exclude Stripe IDs.

### 7. Rate Limiter Resets on Cold Start
**Issue:** In-memory rate limit store lost on serverless cold start.
**Fix:** `@upstash/ratelimit` and `@upstash/redis`. When `UPSTASH_REDIS_REST_URL` and `UPSTASH_REDIS_REST_TOKEN` are set, use Upstash; otherwise in-memory fallback. `rateLimit()` is now async; all callers use `await rateLimit(...)`.

**Upstash Duration type:** `Ratelimit.slidingWindow(maxRequests, window)` expects `Duration` (`"10 s"`, `"15 m"`). TypeScript infers `string` from template literals; use `as Duration` or return type `Duration` when building window strings from ms.

### 8. robots.txt and sitemap.xml
**Issue:** Not present (referenced in marketing docs).
**Fix:** `app/robots.ts` - User-agent: *, Allow: /, Disallow: /dashboard, /api, Sitemap URL. `app/sitemap.ts` - static pages + dynamic `/legal-info/[slug]` from LegalContent DB.

### 9. Payment Failure Emails
**Issue:** `invoice.payment_failed` webhook only created Payment record; no email to user.
**Fix:** In `handlePaymentFailed`, after creating Payment: fetch user email from Subscription→User; create Stripe billing portal session; call `sendPaymentFailedEmail(userEmail, portalUrl)` with link to update payment method.

### requireAdmin Token Null Check
**Issue:** TypeScript error: `'token' is possibly 'null'` when accessing `token.sub` after `!token?.sub` check.
**Fix:** Add explicit `if (!token) return { user: null, error: ... }` before the `!token.sub` check so TypeScript narrows token to non-null.

**Lesson:** Phase 1 fixes address security and data integrity gaps before growth features. Set `UPSTASH_REDIS_REST_URL` and `UPSTASH_REDIS_REST_TOKEN` in production for persistent rate limiting. After migration, promote at least one user to admin role.

---

## Phase 2 Growth Features (March 2026)

### 2.1 New User Onboarding Flow
- Added `hasCompletedOnboarding` to User model
- 3-step wizard: overview, situation, recommended first step (Start Petition)
- `OnboardingWrapper` shows wizard on first dashboard visit; `complete-onboarding` API marks done
- `GettingStartedChecklist` for new users: Complete petition, Add case info, Generate documents

### 2.2 Personalized Case Insights Dashboard
- `CaseInsightsCard` on dashboard: cost estimate, timeline, case readiness score
- `getCaseInsightsForUser()` derives county, type, hasChildren, hasProperty from CaseInfo, Child, petition, financialData
- `getCaseReadinessScore()` weights questionnaires 40%, documents 35%, milestones 25%

### 2.3 Open Graph and SEO Metadata
- Root layout: openGraph, twitter cards, keywords
- Per-page metadata: pricing, contact, calculators, legal-info
- `generateMetadata` for legal-info/[slug] with title, description from LegalContent

### 2.4 Notification Preferences
- `UserNotificationPreferences` model: deadlineReminders, marketingEmails, documentNotifications
- Migration `20260306000000_phase2_onboarding_and_notifications`
- GET/PATCH `/api/auth/notification-preferences`
- Profile page: NotificationPreferences component with checkboxes
- Deadline-reminders cron checks `deadlineReminders` before sending

**Lesson:** Run `npx prisma migrate deploy` to apply Phase 2 migrations (hasCompletedOnboarding, user_notification_preferences).
